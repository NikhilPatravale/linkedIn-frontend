...
function Messaging() {
  ...
  useEffect(() => {
    ...
    if (showConversationList) fetchConversations();
  }, [showConversationList]);

  useEffect(() => {
    const updateShowConversationList = () => setShowConversationList(
      window.innerWidth >= 1024 || !location.pathname.includes("conversations")
    );

    window.addEventListener("resize", updateShowConversationList);

    return () => window.removeEventListener("resize", updateShowConversationList);
  }, [location.pathname]);
  ...
  return (
    <div className={classes.root}>
      <div className={classes.messagingContainer}>
        {showConversationList && (<div className={classes.messagingContainerLeft}>
          ...
          <div className={classes.messageList}>
            ...
            {conversations.length > 0 && <div className={classes.conversationList}>
              {conversations.map(
                (conversation) => (
                  <Conversation
